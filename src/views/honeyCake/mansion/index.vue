<template>
  <div id="mainWindow">
    <h3>大厦信息</h3>
    <el-form
      ref="infoForm" :model="mansionForm" :rules="mansionRules"
      label-width="100px"
      label-position="left"
    >
      <el-form-item label="大厦号" prop="id">
        <el-input
          v-model="mansionForm.id" placeholder="请输入大厦号" class="width30"
          @blur="updateId(mansionForm.id)"
        />
      </el-form-item>
      <el-form-item label="描述" prop="description">
        <el-input
          v-model="mansionForm.description" placeholder="请输入描述"
          class="width30"
        />
      </el-form-item>
      <el-form-item label="CV号" prop="cv_link">
        <el-input
          v-model="mansionForm.cv_link" placeholder="请输入CV号"
          class="width30"
        >
      ...
      <el-form-item label="自信度" prop="fraction">
        <el-slider
          v-model="mansionForm.fraction" class="fraction-slider" :min="1"
          :max="5" :step="1"
        />
      </el-form-item>
    </el-form>
    <h3>每日信息</h3>
    <draggable
      tag="el-collapse" handle=".collapse-header" :list="mansionForm.daily"
      :component-data="collapseComponentData" @start="draggStart" @end="draggEnd"
    >
      <el-collapse-item
        v-for="(item, index) in mansionForm.daily" :key="index" :name="index"
        class="btn"
      >
        <template slot="title">
        ...
        <el-card class="single-card">
          <el-form
            :ref="'dailyForm' + index" :model="mansionForm.daily[index]" :rules="mansionRules"
            class="item-list"
            label-width="100px" label-position="left"
          >
            <el-form-item label="日期" prop="datetime">
              <el-date-picker
                v-model="item.datetime" type="datetime" placeholder="选择开始显示日期时间"
                align="center"
            ...
            <el-form-item label="预测详细" prop="forecast">
              <div
                v-for="(detail, i) in item.info" :key="i"
                class="forecast-info"
              >
                <el-input
                  v-model="detail.forecast" placeholder="请输入预测信息" class="width50"
                ...
                <el-button
                  icon="el-icon-plus" class="btn-editor btn-addForecast"
                  @click.stop="addForecast(index, i)"
                />
                <el-button
                  icon="el-icon-delete" class="btn-editor btn-deleteForecast"
    ...
    <div class="id-option">
      <el-select
        v-model="selectIdShow" label="大厦号" placeholder="请选择大厦号"
        @change="changeId"
      >
        <el-option
          v-for="item in idOptions" :key="item.value" :label="item.label"
          :value="item.value"
        />
      </el-select>
      <el-button
    ...
    <form-button @submit="submitMansionList()" />
  </div>
</template>
<script>
...
export default {
    ...
    // 几组默认值
        ...
        mansionList.cv_link !== ""
            ...
            mansionList.daily.forEach((item, _) => {
